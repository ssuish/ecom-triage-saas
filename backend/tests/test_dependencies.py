...
def test_require_magic_token_header_raises_401_when_missing(db_session):
    ...
    request.headers = {}
    with pytest.raises(HTTPException) as exc:
        require_magic_token_header(request=request, db=db_session)
    assert exc.value.status_code == 401
...
def test_require_magic_token_header_passes_valid_token(db_session):
    ...
    request = MagicMock()
    request.headers = {"x-magic-token": "header-token-uuid"}
    result = require_magic_token_header(request=request, db=db_session)
